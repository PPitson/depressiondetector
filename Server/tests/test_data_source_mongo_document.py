...
class DataSourceMongoDocumentTestCase(CustomTestCase):
    ...
    def test_computes_average_of_happiness_after_save(self):
        now = datetime.utcnow()
        yesterday = now - timedelta(days=1)
        self.assertEqual(HappinessLevel.objects.count(), 0)
        EmotionFromTextExtractionResult.objects.create(user=self.user, datetime=yesterday, surprise=0.1, joy=0.4,
                                                       sadness=0.1, anger=0.1, fear=0.3)
        EmotionExtractionResult.objects.create(user=self.user, datetime=yesterday, neutral=0.1, happy=0.2, sad=0.2,
                                               angry=0.1, fear=0.4)

        self.assertEqual(HappinessLevel.objects.count(), 1)
        self.assertEqual(HappinessLevel.objects.first().text_happiness_level, 0.4)
        self.assertEqual(HappinessLevel.objects.first().voice_happiness_level, 0.2)
        self.assertAlmostEqual(HappinessLevel.objects.first().mean_happiness_level, 0.3)

        EmotionFromTextExtractionResult.objects.create(user=self.user, datetime=yesterday, surprise=0.3, joy=0.55,
                                                       sadness=0.5, anger=0.07, fear=0.03)
        EmotionExtractionResult.objects.create(user=self.user, datetime=yesterday, neutral=0.3, happy=0.55, sad=0.5,
                                               angry=0.07, fear=0.03)

        self.assertEqual(HappinessLevel.objects.count(), 1)
        self.assertAlmostEqual(HappinessLevel.objects.first().text_happiness_level, 0.475)
        self.assertAlmostEqual(HappinessLevel.objects.first().voice_happiness_level, 0.375)
        self.assertAlmostEqual(HappinessLevel.objects.first().mean_happiness_level, 0.425)
